...
2.1 challenges (챌린지)
--------------------------------------------------------------------------------
※ 주의: 테이블명은 "challenges"입니다. "gye"는 레거시 용어로, 실제 DB에서는 사용되지 않습니다.
...
[컬럼값 정의]
  - type   : KICK(멤버강퇴), LEADER_KICK(리더탄핵), DISSOLVE(챌린지해산)
  - status : PENDING(진행중), APPROVED(승인), REJECTED(거절), EXPIRED(만료)
