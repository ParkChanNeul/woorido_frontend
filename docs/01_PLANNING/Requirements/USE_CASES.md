...
6. 자동으로 계좌 생성 (accounts 테이블)
   - balance: 0
   - locked_balance: 0
7. user_scores 레코드 생성 (초기 점수: 36.5)
...
> 관련 테이블: `gye`, `gye_members`, `ledger_entries`

### UC-CHALLENGE-01: 챌린지 생성

**액터**: 로그인 유저 (리더가 되려는 자)
**전제조건**: accounts 잔액 충분 (보증금)
**관련 테이블**: `gye`, `gye_members`
...
**사후조건**:
- `gye` 레코드 생성 (status: RECRUITING)
- `gye_members` 레코드 생성 (role: LEADER)
...
4. "확인" 클릭
5. 트랜잭션 처리
   - gye.current_members 증가 (Optimistic Lock + version)
   - gye_members 레코드 생성 (role: 'FOLLOWER')
...
**사후조건**:
- `gye.current_members` 증가
- `gye_members` 레코드 생성
- (3명 달성 시) `gye.status = 'ACTIVE'`
...
입회비 = 챌린지 금고 잔액 / (현재 멤버 수 - 1)
       = gye.balance / (gye.current_members - 1)
```
...
6. 트랜잭션 처리 (@Transactional)
   ...
   - gye.current_members 증가 (Optimistic Lock)
   - gye_members 레코드 생성

**사후조건**:
- 개인 `accounts.balance` 감소
- 개인 `accounts.locked_balance` 증가 (보증금)
- 챌린지 `gye.balance` 증가 (입회비)
...
**액터**: 팔로워
**전제조건**: gye_members.left_at IS NULL
**관련 테이블**: `gye`, `gye_members`, `accounts`, `account_transactions`
...
4. 트랜잭션 처리
   ...
   - gye_members.leave_reason = 'NORMAL'
...
**액터**: 시스템 (배치)
**트리거**: 매월 1일 00:00
**관련 테이블**: `accounts`, `account_transactions`, `gye`, `gye_members`, `ledger_entries`
...
4. 정상 납입 처리
   ...
   - gye_members.last_support_paid_at 갱신
...
2. 보증금 충당 (안심결제)
   ...
   - 서포트 납입 완료
3. 권한 박탈 처리
   - gye_members.privilege_status → 'REVOKED'
   - gye_members.privilege_revoked_at 기록
...
2. 자동 탈퇴 처리
   - gye_members.left_at 기록
   - gye_members.leave_reason = 'AUTO_LEAVE_DEPOSIT_NOT_RECHARGED'
   - gye.current_members 감소
3. 리더에게만 알림
...
- `votes.status = 'APPROVED'`
- `gye_members.leave_reason = 'KICKED'`
- `user_scores.total_kick_count` 증가 (-4.0점)
...
8. 70% 이상 찬성 시 (@Transactional)
   ...
   - 리더 승계 처리
     ...
     - 없으면 → user_scores.total_score 최고자에게 양도
   - 기존 리더 → FOLLOWER로 전환
